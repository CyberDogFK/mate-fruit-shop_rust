...
#[test]
fn write_with_title() {
    let csv_writer = CsvWriter::new(Some(TITLE_TO_FILE.to_string()));
    let file_name = Uuid::new_v4().to_string();
    csv_writer
        .save_to_file(&file_name, TWO_LINE_TO_FILE.to_string())
        .unwrap();
...
fn rewrite_lines_from_existed_file() {
    let file_name = Uuid::new_v4().to_string();
    {
        ...
        wri.write_all(ONE_LINE_TO_FILE_EXAMPLE_1.as_bytes())
            .unwrap();
    }
    ...
    let _ =
        CsvWriter::new(None).save_to_file(&test_file.name, ONE_LINE_TO_FILE_EXAMPLE_2.to_string());
