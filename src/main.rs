use fruit_shop_rust::service::{create_report, csv_reader, parse, CsvWriter};
use fruit_shop_rust::strategy::Operation::{BALANCE, PURCHASE, RETURN, SUPPLY};
use fruit_shop_rust::strategy::{
    AdderHandler, FruitTransaction, Operation, ReduceHandler, SavedHandler, TransactionHandler,
};
use fruit_shop_rust::FruitStorage;
...
fn main() {
    ...
    let strings =
        csv_reader::read_file_with_skip_lines(EXAMPLE_FILE, NUMBER_OF_TITLE_IN_CSV).unwrap();
...
fn apply_transactions(
    transactions: Vec<FruitTransaction>,
    fruit_storage: Rc<RefCell<FruitStorage>>,
) {
...
fn map_transactions(
    fruit_storage: Rc<RefCell<FruitStorage>>,
) -> HashMap<Operation, Box<dyn TransactionHandler>> {
